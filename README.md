...
- [ ] borard response dto를 그냥, board entity의 데이터만 옮겨서 쓰는것보단, 좀더 가공하는데 어떻게가공할까. boardResponseDto, boardsResponseDto두개를 만들어서 게시판 데이터 getBoardById로 응답할 때랑 getAllBoards로 응답할때랑 다른 형식으로 할까? 아니면 그냥 boardResponseDto 타입 하나를 통해서 data : {BoardResponseDto},혹은 count : {number}, data : [{BoardResponseDto}, ... ]}로 응답할까?
